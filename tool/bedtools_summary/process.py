    ...
    def _get_sample_info_rna(self):
        ...
        with self.db_connection.create_session() as session:
            ...
            if self.sample_attr is None:
                print('Please check table DatasetSeqRunSample, DatasetSeqRun, dbSample, SampleAttribute, SampleGroup! There is something wrong with the database')
    ...
    def write_downsample_meta(self, dset, s_stats: Dict):
        ...
        for k,v in group_samples.items():
            ...
            for vi in v:
                ...
                if s_stats[vi].r16 is not None and s_stats[vi].r17 is not None:
                    downsample_meta[vi]['adj_exp_reads_a'] = round(s_stats[vi].r17)
                    ...
                    downsample_meta[vi]['adj_exp_reads_a'] = None
                    downsample_meta[vi]['down_million_a'] = None
                    downsample_meta[vi]['frac_a'] = None


                if s_stats[vi].r15 is not None and s_stats[vi].r18 is not None:
                    downsample_meta[vi]['adj_exp_reads_b'] = round(s_stats[vi].r18)
                    ...
                    downsample_meta[vi]['adj_exp_reads_b'] = None
                    downsample_meta[vi]['down_million_b'] = None
                    downsample_meta[vi]['frac_b'] = None

            if l_a is not None:
                for vi in v:
                    if s_stats[vi].r16 is not None:
                        downsample_meta[vi]['adj_exp_reads_a'] = round(s_stats[vi].r4 * (s_stats[vi].r16 / l_a))
                        downsample_meta[vi]['down_million_a'] = str(round(s_stats[vi].r4 * (s_stats[vi].r16 /l_a) / 1000000, 1)) + 'M'
                        downsample_meta[vi]['frac_a'] = s_stats[vi].r16 / l_a
                    else:
                        downsample_meta[vi]['adj_exp_reads_a'] = round(s_stats[vi].r4 * (1 / l_a))
                        downsample_meta[vi]['down_million_a'] = str(round(s_stats[vi].r4 * (1 /l_a) / 1000000, 1)) + 'M'
                        downsample_meta[vi]['frac_a'] = 1 / l_a


            if l_b is not None:
                for vi in v:
                    if s_stats[vi].r15 is not None:
                        downsample_meta[vi]['adj_exp_reads_b'] = round(s_stats[vi].r4 * (s_stats[vi].r15 / l_b))
                        downsample_meta[vi]['down_million_b'] = str(round(s_stats[vi].r4 * (s_stats[vi].r15 / l_b) / 1000000, 1)) + 'M'
                        downsample_meta[vi]['frac_b'] = s_stats[vi].r15 / l_b
                    else:
                        downsample_meta[vi]['adj_exp_reads_b'] = round(s_stats[vi].r4 * (1 / l_b))
                        downsample_meta[vi]['down_million_b'] = str(round(s_stats[vi].r4 * (1 / l_b) / 1000000, 1)) + 'M'
        ...
        with open('metaconfig/chipseq_downsample_meta.yaml', 'a') as f:
            ...
            for k,v in downsample_meta.items():
                ...
                if v['down_million_a'] is None:
                    f.write('    down_million_a:\n' + '      ' + str(v['down_million_a']) + '\n')
                else:
                    f.write('    down_million_a:\n' + '      \"' + v['down_million_a'] + '\"\n')
                f.write('    frac_a:\n' + '      ' + str(v['frac_a']) + '\n')
                f.write('    adj_exp_reads_b:\n' + '      ' + str(v['adj_exp_reads_b']) + '\n')
                if v['down_million_b'] is None:
                    f.write('    down_million_b:\n' + '      ' + str(v['down_million_b']) + '\n')
                else:
                    f.write('    down_million_b:\n' + '      \"' + v['down_million_b'] + '\"\n')
                f.write('    frac_b:\n' + '      ' + str(v['frac_b']) + '\n')
                f.write('    adj_exp_reads_r:\n' + '      \"' + str(v['adj_exp_reads_r']) + '\"\n')
                f.write('    down_million_r:\n' + '      \"' + str(v['down_million_r']) + '\"\n')
                f.write('    frac_r:\n' + '      ' + str(v['frac_r']) + '\n')
            f.write('\n')
