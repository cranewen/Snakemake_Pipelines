...
# all the related classes are in data.py
# any function starts with '_' is the database version
# the main task for Database class is to form a "sample_meta" dictionary {cpct_full_name: sample object -> imported from data}
class Database:
    def __init__(self, dataset):
        ...
        self.sample_meta_shortname: Dict = defaultdict() # a dict {sample short name: Sample object}
    ...
    # get rna info from database without flagstat info
    def _get_sample_info_rna(self):
        ...
        with self.db_connection.create_session() as session:
            ...
            group_color_dict = defaultdict()
            n = len(all_colors)
            print(f'color n:=== {n}')
            for i in range(n):
                group_color_dict[i + 1] = all_colors[i]
            ...
            for s in self.sample_attr:
                ...
                if s_n == s[0].fastqSampleNumber:
                    ...
                    #    sample.color = group_color_dict[s[4].sampleGroupBaseColor]
                    
                    color_ind = 15 if s[0].sampleGroupID % 15 == 0 else s[0].sampleGroupID % 15
                    print(f'color_ind is :=== {color_ind}')
                    sample.color = group_color_dict[color_ind]
                    sample._update_rna_input() # because RNAseq doesn't have inputs, so this function sets all the inputs to false
            ...
            print(self.sample_meta_shortname)
...
# after parsing the data from database, the sample_meta has been generated, so we pass sample_meta into this class
# all the functions modifiy the db.sample_meta which afterwards becoming the return object
class DataParser:
    def __init__(self, dataset: Dataset):
        ...
        # self.sample_meta isn't initialized right away, all the functions in DataParser modifies the sample_meta from db.sample_meta
        # later, self.sample_meta saves the finalized db.sample_meta as the return object or its own class object
    ...
    # not been used in current version
    ...
    # parsing data from sample_meta.yaml (completely disconnected from database)
...
# the class for writing info to sample_meta.yaml
class DataWriter:
    ...
    # for RNAseq
    def write_rna_sample_meta(self):
        ...
        # checking dataset if exists to prevent writing redundent info
    ...
    # s_stats is from calculate_sample_stats()
    # r16 and r17 are paired for a, r15 and r18 are paired for b
    def write_downsample_meta(self, dset, s_stats: Dict):
        for k,v in s_stats.items():
            print(v)
        group_samples = defaultdict() # {group_id: [group cpct full name list]}
        lowest_sample = defaultdict() # {group_id:lowest counts sample}
        downsample_meta = defaultdict()
        for k,v in s_stats.items():
            try:
                group_samples[v.s.group_id].append(v.s.cpct_full_name)
            except KeyError:
                group_samples[v.s.group_id] = []
                group_samples[v.s.group_id].append(v.s.cpct_full_name)
        ...
        for k,v in group_samples.items():
            ...
            l_r = None
            low_mark = None
            s_a = '' # record lowest sample for method a
            s_b = '' # record lowest sample for method b
            s_r = '' # record lowest sample for raw

            for vi in v:
                downsample_meta[vi] = defaultdict()
                downsample_meta[vi]['group_id'] = k
                downsample_meta[vi]['rumdup_aligned_reads'] = s_stats[vi].r4
                if l_r is None:
                    l_r = s_stats[vi].r4
                if s_stats[vi].r4 < l_r:
                    l_r = s_stats[vi].r4
                    s_r = vi

                if s_stats[vi].r16 is not None and s_stats[vi].r17 is not None:
                    downsample_meta[vi]['adj_exp_reads_a'] = s_stats[vi].r17
                    downsample_meta[vi]['down_million_a'] = str(round(s_stats[vi].r17 / 1000000, 1)) + 'M'
                    downsample_meta[vi]['frac_a'] = s_stats[vi].r17 / s_stats[vi].r4
                    if s_stats[vi].r16 > 1:
                        if l_a is not None:
                            if s_stats[vi].r16 > l_a:
                                l_a = s_stats[vi].r16
                                s_a = vi
                        else:
                            l_a = s_stats[vi].r16
                ...
                if s_stats[vi].r15 is not None and s_stats[vi].r18 is not None:
                    downsample_meta[vi]['adj_exp_reads_b'] = s_stats[vi].r18
                    downsample_meta[vi]['down_million_b'] = str(round(s_stats[vi].r18 / 1000000, 1)) + 'M'
                    downsample_meta[vi]['frac_b'] = s_stats[vi].r18 / s_stats[vi].r4
                    if s_stats[vi].r15 > 1:
                        if l_b is not None:
                            if s_stats[vi].r15 > l_b:
                                l_b = s_stats[vi].r15
                                s_b = vi
                        else:
                            l_b = s_stats[vi].r15
            ...
            if l_a is not None:
                for vi in v:
                    downsample_meta[vi]['adj_exp_reads_a'] = s_stats[vi].r4 / l_a
                    downsample_meta[vi]['down_million_a'] = str(round(s_stats[vi].r4 / l_a / 1000000, 1)) + 'M'
                    downsample_meta[vi]['frac_a'] = 1 / l_a

            if l_b is not None:
                for vi in v:
                    downsample_meta[vi]['adj_exp_reads_b'] = s_stats[vi].r4 / l_b
                    downsample_meta[vi]['down_million_b'] = str(round(s_stats[vi].r4 / l_b / 1000000, 1)) + 'M'
                    downsample_meta[vi]['frac_b'] = 1 / l_b

            for vi in v:
                downsample_meta[vi]['adj_exp_reads_r'] = l_r
                downsample_meta[vi]['down_million_r'] = str(round(l_r / 1000000, 1)) + 'M'
                downsample_meta[vi]['frac_r'] = l_r / s_stats[vi].r4
        ...
        # for k,v in group_samples.items():
        #     lowest_sample[k] = defaultdict()

        #     lowest_a = 'a'
        #     lowest_b = 'b'
        #     lowest_r = s_stats[v[0]].r4
        #     ratios_a = defaultdict()
        #     ratios_b = defaultdict()
        #     temp_a = None
        #     temp_b = None
        #     temp_r = None
        #     r16_mark = s_stats[v[0]].r16 # mark for first r16 
        #     r15_mark = s_stats[v[0]].r15 # mark for first r15
        #     ratio_mark_a = 0
        #     ratio_mark_b = 0
        #     for vi in v:
        #         # get lowest for a
        #         if s_stats[vi].r16 is not None and s_stats[vi].r17 is not None:
        #             ratios_a[vi] = s_stats[vi].r16
        #             print(f'sample name:=== {vi}')
        #             if ratio_mark_a == 0:
        #                 ratio_mark_a = s_stats[vi].r16
        #                 temp_a = vi
        #             if ratio_mark_a >= 1:
        #                 if s_stats[vi].r16 > ratio_mark_a:
        #                     lowest_a = s_stats[vi].r17
        #                     ratio_mark_a = s_stats[vi].r16
        #                     temp_a = vi
        #                 # else:
        #                 #     ratio_mark_a = r16_mark
        #             else:
        #                 if s_stats[vi].r17 <= lowest_a:
        #                     lowest_a = s_stats[vi].r17
        #                     temp_a = vi
                    
        #         # get lowest for b
        #         if s_stats[vi].r15 is not None and s_stats[vi].r18 is not None:
        #             ratios_b[vi] = s_stats[vi].r15
        #             print(f'sample name:=== {vi}')
        #             if ratio_mark_b == 0:
        #                 ratio_mark_b = s_stats[vi].r15
        #                 temp_b = vi
        #             if ratio_mark_b >= 1:
        #                 if s_stats[vi].r15 > ratio_mark_b:
        #                     lowest_b = s_stats[vi].r18
        #                     ratio_mark_b = s_stats[vi].r15
        #                     temp_b = vi
        #                 # else:
        #                 #     ratio_mark_b = r15_mark
        #             else:
        #                 if s_stats[vi].r18 <= lowest_b:
        #                     lowest_b = s_stats[vi].r18
        #                     temp_b = vi

        #         if s_stats[vi].r4 <= lowest_r:
        #             temp_r = vi
        #             lowest_r = s_stats[vi].r4
           
        #     lowest_sample[k]['a'] = temp_a
        #     lowest_sample[k]['b'] = temp_b
        #     lowest_sample[k]['r'] = temp_r
        # print(f'lowest_sample ==> {lowest_sample}') 
        # print(f'group_samples ==> {group_samples}')
        ...
        #     for vi in v:
        #         downsample_meta[vi] = defaultdict()
        #         downsample_meta[vi]['group_id'] = s_stats[vi].s.group_id
        #         downsample_meta[vi]['rmdup_aligned_reads'] = s_stats[vi].r4
        #         frac_a = 0
        #         frac_b = 0
        #         frac_r = 0
        #         if s_stats[vi].r17 is not None and s_stats[vi].r16 is not None:
        #             frac_a = s_stats[lowest_sample[k]['a']].r17 / s_stats[vi].r17
        #         if s_stats[vi].r18 is not None and s_stats[vi].r15 is not None:
        #             frac_b = s_stats[lowest_sample[k]['b']].r18 / s_stats[vi].r18
                
        #         frac_r = s_stats[lowest_sample[k]['r']].r4 / s_stats[vi].r4
        #         downsample_meta[vi]['frac_a'] = frac_a
        #         downsample_meta[vi]['frac_b'] = frac_b
        #         downsample_meta[vi]['frac_r'] = frac_r
        #         downsample_meta[vi]['adj_exp_reads_a'] = s_stats[vi].r17
        #         downsample_meta[vi]['adj_exp_reads_b'] = s_stats[vi].r18
        #         downsample_meta[vi]['down_million_a'] = str(round(s_stats[vi].r4 * frac_a / 1000000, 1)) + 'M'
        #         downsample_meta[vi]['down_million_b'] = str(round(s_stats[vi].r4 * frac_b / 1000000, 1)) + 'M'
        #         downsample_meta[vi]['down_million_r'] = str(round(s_stats[vi].r4 * frac_r / 1000000, 1)) + 'M'
        ...
        for k,v in group_samples.items():
            for vi in v:
                ...
                frac = 0
                if s_stats[vi].r17 is not None and s_stats[vi].r16 is not None:
                    print(f'lowest sample:=={lowest_sample[k]}')
                    frac = s_stats[lowest_sample[k]['a']].r17 / s_stats[vi].r17
                elif s_stats[vi].r18 is not None and s_stats[vi].r15 is not None:
                    frac = s_stats[lowest_sample[k]['b']].r18 / s_stats[vi].r18
                else:
                    frac = s_stats[lowest_sample[k]['r']].r4 / s_stats[vi].r4
                downsample_meta[vi]['frac'] = frac
                downsample_meta[vi]['down_million'] = str(round(s_stats[vi].r4 * frac / 1000000, 1)) + 'M'
        '''
        # ic(downsample_meta)

        sample_meta_check = YamlReader('metaconfig/chipseq_downsample_meta.yaml')
        for smc in sample_meta_check.read_yaml():
            try:
                if smc.get(dset.dataset_name):
                    print(f'{dset.dataset_name} exists!')
                    return 0
            except KeyError:
                print(f'Writing {dset.dataset_name} sample meta!')

        '''
        with open('metaconfig/chipseq_downsample_meta.yaml', 'a') as f:
            f.write(dset.dataset_name + ':\n')
            for k,v in downsample_meta.items():
                f.write('  ' + k + ':\n')
                f.write('    group_id:\n' + '      ' + str(v['group_id']) + '\n')
                f.write('    rmdup_aligned_reads:\n' + '      ' + str(v['rmdup_aligned_reads']) + '\n')
                f.write('    adj_exp_reads_a:\n' + '      ' + str(v['adj_exp_reads_a']) + '\n')
                f.write('    down_million_a:\n' + '      \"' + v['down_million_a'] + '\"\n')
                f.write('    frac_a:\n' + '      ' + str(v['frac_a']) + '\n')
                f.write('    adj_exp_reads_b:\n' + '      ' + str(v['adj_exp_reads_b']) + '\n')
                f.write('    down_million_b:\n' + '      \"' + v['down_million_b'] + '\"\n')
                f.write('    frac_b:\n' + '      ' + str(v['frac_b']) + '\n')
                f.write('    down_million_r:\n' + '      \"' + v['down_million_r'] + '\"\n')
                f.write('    frac_r:\n' + '      ' + str(v['frac_r']) + '\n')
            f.write(':\n')
        '''
...
class DF:
    ...
    # ===== notes for downsampling pipeline =====
    # if no flyreads pick the min(r4), then calculate the ratios the least counts sample has ratio 1, because it doesn't change
    # if flyreads pick min(r16 'method a') or min(r17 'method b'), then calculate the ratios
    # r14 for 'a', r15 for 'b', if there is any > 1, look for the largest r14 and re-calculate the ratio, otherwise pick the lowest r14 or r15 

    def calculate_sample_stats(self, samples: Dict):
        s_dict = defaultdict() # {cpct_full_name: SampleStat object}
        # SampleStat has "s ==> sample object" with r1-r35 object members
