...
class DataWriter:
    ...
    def write_downsample_meta(self, dset, s_stats: Dict):
        ...
        for k,v in group_samples.items():
            ...
            temp = ''
            for vi in v:
                if s_stats[vi].r17 is not None:
                    if s_stats[vi].r17 <= lowest:
                        lowest = s_stats[vi].r17
                        temp = vi
                elif s_stats[vi].r18 is not None:
                    if s_stats[vi].r18 <= lowest:
                        lowest = s_stats[vi].r18
                        temp = vi
                elif s_stats[vi].r4 is not None:
                    if s_stats[vi].r4 <= lowest:
                        lowest = s_stats[vi].r4
                        temp = vi
            
            lowest_sample[k] = temp
        ...
        for k,v in group_samples.items():
            for vi in v:
                ...
                frac = 0
                if s_stats[vi].r17 is not None:
                    frac = s_stats[lowest_sample[k]].r17 / s_stats[vi].r17
                elif s_stats[vi].r18 is not None:
                    frac = s_stats[lowest_sample[k]].r18 / s_stats[vi].r18
                else:
                    frac = s_stats[lowest_sample[k]].r4 / s_stats[vi].r4
